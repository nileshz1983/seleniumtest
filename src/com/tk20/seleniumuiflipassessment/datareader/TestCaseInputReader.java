...
public class TestCaseInputReader {
	...
	**/
 	...
 	public String getSheetNameAtIndex(int sheetIndex){
 	    ...
 	    /**
		* Added by Nitin Gupta on 28th March 2016
		* This Method is used to delete all the extra rows which contains unused data
		**/
    
 	public void delAllExtraRows(String sheetName,int rowCountToDel)
 	{
 	    try {
 	    	int index = workbook.getSheetIndex(sheetName);
 	    	sheet = workbook.getSheetAt(index);
 	    	
 	    	for(int i=rowCountToDel;i<=sheet.getLastRowNum();i++)
 	    	{
	...
	* This Method is used to delete all the extra columns which contains unused data
	**/
     
     
     public void deleteAllExtraColumns(String sheetName,int columnCountToDel,int rowCountToDel){
 		try{
 			...
 			sheet = workbook.getSheetAt(index);
 					...
 					col_Num=columnCountToDel;
 					
 					for(int i=2;i<=rowCountToDel;i++)
 					{
 						Row row=sheet.getRow(i-1);
 					for(int j=col_Num;j<row.getLastCellNum();j++)
 					{
 					cell = row.getCell(j);
     ...
     /**
 	* Added by Shweta Gupta on 17 November 2016
 	* This Method is used to get data from data column whose values are specified in OR files.
 	**/
      
     public Set<String> getAllORDataFromXls(String sheetName, String colName, String xls_Suite , int total_suites, int suiteid) {
	        Set<String> xpaths = new HashSet<String>();
	        int flag=0;
	        try {
	            int index = workbook.getSheetIndex(sheetName);
	                ...
	                if (row.getCell(i).getStringCellValue().trim().equals(colName.trim())) {
	                    ...
	                    while (rowIter.hasNext()) {
	                        ...
	                        if (!StringUtils.isEmpty(Data)) {
	                            if (Data.contains("|")) //Added code to check objects with "|"
	                            	continue;
	                           else
	                                xpaths.add(Data);
	                        }
